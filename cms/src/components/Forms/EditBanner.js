...
import {EditPutBanner} from '../../hooks/ApiRequest'
import { toast } from 'react-toastify';
import {SuccesAlert, ErrorAlert} from '../Alert/Alert'
import {useDispatch} from 'react-redux';
import {fetchBanner} from '../../store/actions';

function EditBanner ({handleEditBanner, populateData}) {
  const [inputUser, setInputUser] = useState({
    image_url: populateData.image_url,
    detail: populateData.detail,
  }) 
  ...
  function handleEdit(){
    ...
    EditPutBanner(payload)
      .then(response => {
        successNotif("Banner Has Been Updated");
                    ...
                    <input 
                      ...
                      onChange={(e)=>{
                        ...
                        fetch("https://api.imgur.com/3/image/",{
                          ...
                          if(data.success){
                            setInputUser({...inputUser, image_url: data.data.link})
                          } else {
                            setInputUser({...inputUser, image_url: inputUser.image_url})
                          }
                        }).catch(err =>{
